...
try:
    ...
    print("Warning: librosa not installed. Some features may be disabled.")
import click
import shutil
import warnings
...
try:
    ...
    print("Warning: tensorboard not installed. Training will continue without tensorboard logging.")
...
def main(config_path):
    ...
    train_num_workers = max(0, int(loader_params.get('train_num_workers', 2)))
    val_num_workers = max(0, int(loader_params.get('val_num_workers', 0)))
    ...
    if load_pretrained:
        ...
        print(f"DEBUG: Configured pretrained_model path: {pretrained_path}")
        print(f"DEBUG: Absolute path: {osp.abspath(pretrained_path)}")
        print(f"DEBUG: File exists: {osp.exists(pretrained_path)}")
        ...
        if not osp.exists(pretrained_path):
            ...
            alt_path = osp.join(osp.dirname(__file__), pretrained_path)
            print(f"DEBUG: Trying alternative path: {alt_path}")
            print(f"DEBUG: Alternative path exists: {osp.exists(alt_path)}")
            if osp.exists(alt_path):
                pretrained_path = alt_path
                print(f"DEBUG: Using alternative path: {pretrained_path}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if epoch >= diff_epoch:
                ...
                diffusion_module = model.diffusion.module if isinstance(model.diffusion, torch.nn.DataParallel) else model.diffusion
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                try:
                    ...
                    for bib in range(len(mel_input_length)):
                        ...
                        # Skip if sample is too short for clipping
        ...
        if iters_test > 0:
            ...
            logger.warning('No validation batches succeeded - skipping validation metrics')
        print('\n\n\n')
        if writer is not None and iters_test > 0:
